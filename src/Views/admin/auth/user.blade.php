                ...
                <div class="card-header with-border">
                    ...
                    <div class="card-tools">
                        <div class="btn-group float-right mr-5">
                            <a href="{{ vncore_route_admin('admin_user.index') }}" class="btn btn-sm  btn-flat btn-default" title="List"><i class="fa fa-list"></i><span class="hidden-xs"> {{ vncore_language_render('admin.back_list') }}</span></a>
                        </div>
                    </div>
                </div>
                <!-- /.card-header -->
                ...
                <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main"  enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                            ...
                            <div class="form-group  row {{ $errors->has('username') ? ' text-red' : '' }}">
                                ...
                                <div class="col-sm-8">
                                    <div class="input-group">
                                        ...
                                        <input type="text"   id="username" name="username" value="{{ old('username',$user['username']??'') }}" class="form-control form-control-sm username" placeholder="" />
                                    </div>
                            ...
                            <div class="form-group  row {{ $errors->has('avatar') ? ' text-red' : '' }}">
                                ...
                                <div class="col-sm-8">
                                    <div class="input-group">
                                        <input type="text" id="avatar" name="avatar" value="{{ old('avatar',$user['avatar']??'') }}" class="form-control form-control-sm avatar" placeholder=""  />
                                       <span class="input-group-btn">
                                         <a data-input="avatar" data-preview="preview_avatar" data-type="avatar" class="btn btn-sm btn-flat btn-primary lfm">
                                           <i class="fa fa-image"></i> {{vncore_language_render('action.choose_image')}}
                                         </a>
                                       </span>
                                    </div>
                                        ...
                                        @if (old('avatar',$user['avatar']??''))
                                            <img src="{{ vncore_file(old('avatar',$user['avatar']??'')) }}">
                                        @endif
                                        </div>
                                </div>
                            </div>
                            ...
                            @if (isset($user['id']) && in_array($user['id'], VNCORE_GUARD_ADMIN))
                                ...
                                @endif
                            @else
                                <select class="form-control form-control-sm roles select2"  multiple="multiple" data-placeholder="{{ vncore_language_render('admin.user.select_roles') }}" style="width: 100%;" name="roles[]" >
                                    <option value=""></option>
                            ...
                            <div class="form-group row {{ $errors->has('permission') ? ' text-red' : '' }}">
                                ...
                                <div class="col-sm-8">
                                    @if (isset($user['id']) && in_array($user['id'], VNCORE_GUARD_ADMIN))
                                        ...
                                        @endif
                                    @else
                                        <select class="form-control form-control-sm permission select2"  multiple="multiple" data-placeholder="{{ vncore_language_render('admin.user.add_permission') }}" style="width: 100%;" name="permission[]" >
                                            <option value=""></option>
                        ...
                        @if (isset($user['id']) && $user['id'] != admin()->user()->id)
                            <div class="form-group row {{ $errors->has('status') ? ' text-red' : '' }}">
                                <label for="status" class="col-sm-2 col-form-label">{!! vncore_language_render('admin.user.status') !!}</label>
                                <div class="col-sm-10 ">
                                <div class="input-group mb-3">
                                    <input class="checkbox" type="checkbox" id="status" name="status"
                                        ...
                                        old('status',(empty($user['status'])?0:1))?'checked':''!!}/>
                                </div>
