             ...
             <table class="table table-hover box-body text-wrap table-bordered">
               <tbody>
                ...
                <tr>
                  ...
                  <td><input class="check-data-config-global"  type="checkbox" name="smtp_mode" {{ vncore_config_global('smtp_mode')?"checked":"" }}></td>
                </tr>
...
<style>
  @if(vncore_config_global('smtp_mode'))
    #smtp-config {
      display:block;
    }
  @else
             ...
             <tbody>
               ...
               @foreach ($emailConfig['smtp_config'] as $config)
                ...
                @elseif($config->key == 'smtp_password' || $config->key == 'smtp_user')
                  <tr>
                    <td>{{ vncore_language_render($config->detail) }}</td>
                    <td align="left"><a href="#" class="editable editable-click" data-name="{{ $config->key }}" data-type="password" data-pk="{{ $config->key }}" data-source="" data-url="{{ $urlUpdateConfig }}" data-title="{{ vncore_language_render($config->detail) }}" data-value="{{ (vncore_admin_can_config()) ? $config->value: 'hidden' }}" data-original-title="" title=""></a></td>
                  </tr>
                @else
                  <tr>
                    <td>{{ vncore_language_render($config->detail) }}</td>
